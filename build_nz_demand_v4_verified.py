...
YEAR = 2025
SCRIPT_VERSION = "2026-07-27-v4-verified"
...
def _read_emi_csv_with_metadata(source) -> pd.DataFrame:
    """Read an EMI CSV that may contain report metadata before the real header.

    EMI report-download endpoints can return several informational lines before
    the comma-separated table. This function detects the actual header row
    instead of relying on a fixed skiprows value.
    """
    ...
    for index, line in enumerate(lines):
        if all(term.lower() in line.lower() for term in required_header_terms):
            header_index = index
            break
    ...
    if header_index is None:
        preview = "\n".join(lines[:12])
        raise ValueError(
            "Could not identify the Network Supply Points CSV header. "
            "The first lines returned by EMI were:\n"
            f"{preview}"
        )
...
def load_network_supply_points() -> pd.DataFrame:
    ...
    if local_file is not None:
        ...
        nsp = _read_emi_csv_with_metadata(local_file)
        ...
        try:
            nsp = _read_emi_csv_with_metadata(NSP_REPORT_URL)
...
def build_daily_poc_mapping(
    ...
    """Map daily Grid_export POC records to model zones.

    Matching is attempted in three stages:
    1. Exact: POC + network participant + reconciliation type
    2. Relaxed: POC + network participant
    3. Fallback: POC only

    In every stage, the NSP record must be active on the trading date.
    """
    key_columns = [
        "Trading_Date",
        "POC",
    ...
    keys = grid_export[key_columns].drop_duplicates().copy()
    keys["_row_id"] = np.arange(len(keys), dtype=np.int64)

    def _select_active_match(
        ...
        if left_rows.empty:
            return pd.DataFrame(
                columns=["_row_id", "nrr_id", "nrr_name", "mapping_stage"]
            )

        candidates = left_rows.merge(
            nsp,
            on=merge_columns,
            how="left",
            suffixes=("", "_nsp"),
        ...
        active = (
            ...
            | candidates["Trading_Date"].ge(candidates["start_date"])
        ) & (
            candidates["end_date"].isna()
            | candidates["Trading_Date"].le(candidates["end_date"])
        )

        selected = (
            candidates.loc[active]
            .sort_values(["_row_id", "start_date"], na_position="first")
            .drop_duplicates("_row_id", keep="last")
        )

        selected = selected.loc[selected["nrr_id"].notna()].copy()
        selected["mapping_stage"] = stage_name
        return selected[["_row_id", "nrr_id", "nrr_name", "mapping_stage"]]

    exact = _select_active_match(
        keys,
        ["POC", "Nwk_Code", "Generation_Type"],
        "exact",
    )

    matched_ids = set(exact["_row_id"])
    remaining = keys.loc[~keys["_row_id"].isin(matched_ids)].copy()

    by_network = _select_active_match(
        remaining,
        ["POC", "Nwk_Code"],
        "poc_network",
    )

    matched_ids.update(by_network["_row_id"])
    remaining = keys.loc[~keys["_row_id"].isin(matched_ids)].copy()

    by_poc = _select_active_match(
        remaining,
        ["POC"],
        "poc_only",
    )
    ...
    selected_all = pd.concat(
        [exact, by_network, by_poc],
        ignore_index=True,
    ).drop_duplicates("_row_id", keep="first")

    daily_map = keys.merge(selected_all, on="_row_id", how="left")

    active_zone_set = set(active_zones)
    daily_map["zone"] = daily_map["nrr_id"].map(
        lambda value: nrr_to_model_zone(value, active_zone_set)
    )

    daily_map = apply_manual_overrides(daily_map, active_zone_set)

    daily_map.loc[
        daily_map["zone"].notna() & daily_map["mapping_stage"].isna(),
        "mapping_stage",
    ] = "manual_override"

    unmapped = daily_map.loc[daily_map["zone"].isna()].copy()
    if not unmapped.empty:
        unmapped.drop(columns="_row_id").to_csv(OUTPUT_UNMAPPED, index=False)
        unique_pocs = unmapped["POC"].nunique()
        raise ValueError(
            f"{len(unmapped):,} daily POC records "
            f"({unique_pocs:,} unique POCs) could not be mapped. "
            f"Review {OUTPUT_UNMAPPED} and add only the remaining POCs to "
            f"{OVERRIDES_FILE}."
        )

    output = daily_map.drop(columns="_row_id")
    output.to_csv(OUTPUT_MAPPING, index=False)

    stage_counts = output["mapping_stage"].fillna("unknown").value_counts().to_dict()
    print(f"POC mapping stages: {stage_counts}")
    return output
...
def main() -> None:
    print(f"Running build_nz_demand version: {SCRIPT_VERSION}")
    RAW_INPUT_DIR.mkdir(
        parents=True,
        exist_ok=True,
    )
